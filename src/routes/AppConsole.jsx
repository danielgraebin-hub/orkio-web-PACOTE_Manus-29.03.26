...
const [onboardingForm, setOnboardingForm] = useState(() => sanitizeOnboardingForm(user));
  ...
  const [speakingAgentId, setSpeakingAgentId] = useState(null); // PATCH_UX: track which agent is speaking for avatar animation
...
useEffect(() => {
  ...
  async function sendMessage(presetMsg = null, opts = {}) {
    ...
    try {
      // PATCH_M: skip prefix if user already typed @mentions inline
      const hasInlineMention = /@[A-Za-z0-9_\-]{2,64}/.test(msg);
      const pref = hasInlineMention ? "" : buildMessagePrefix();
      ...
      setMessages((prev) => [...prev, {
            ...
            // PATCH_N: dynamic placeholder — show correct agent name or "Equipe"
            const placeholderAgentName = destMode === "team" ? "Equipe"
              : destMode === "multi" ? "Agentes"
              : (agents.find(a => a.id === destSingle)?.name || "Orkio");
            setMessages((prev) => [...prev, {
              id: `tmp-ass-${Date.now()}`,
              role: 'assistant',
      ...
      // PATCH_L: always use SSE stream — enables multi-agent responses with separate bubbles
      const shouldUseStream = true;
      let resp = null;
      let newThreadId = threadId;
      let freshMessages = null;
      let streamLastAgentInfo = null;

      // PATCH_ORCH: detect orchestration intent — keywords that trigger Orkio as maestro
      const orchKeywords = /\b(delegue|delegar|distribu[aeiíou]|orquestre|cada\s+um\s+fa[çz]a|plano\s+de\s+neg[oó]cio|montar?\s+.*\s+juntos|equipe\s+toda|todos\s+trabalh|divide|dividir|repartir|cada\s+agente)\b/i;
      const isOrchestration = orchKeywords.test(msg) || /^@orkio\b.*\b(delegue|distribu|orquestre|cada\s+um)/i.test(finalMsg);

      if (shouldUseStream) {
        const streamResp = isOrchestration
        ...
        // PATCH_P: track whether we already removed the optimistic placeholder
        ...
        const applyStreamDraft = (agentId, agentName, content) => {
          ...
          setMessages((prev) => {
            ...
            // PATCH_P: remove optimistic placeholder on first real chunk
            if (!optimisticRemoved) {
              list = list.filter((m) => !String(m?.id || "").startsWith("tmp-ass-"));
        ...
        const processSseBlock = (block) => {
          ...
          if (eventName === "status") {
            ...
            const stepInfo = (step && totalSteps) ? ` (${step}/${totalSteps})` : "";
            const statusText = phase === "planning"
              ? `🧠 ${payload?.status || "Orkio está planejando..."}`
              : `🤖 ${who} pensando...${stepInfo}`;
            try { setUploadStatus(statusText); } catch {}
            return false;
          }

          // PATCH_ORCH: handle orchestration plan event
          if (eventName === "plan") {
            const planSteps = payload?.plan || [];
            const planText = planSteps.map((s, i) => `${i+1}. @${s.agent_name}: ${s.sub_task}`).join("\n");
            try { setUploadStatus(`📋 Plano: ${planSteps.length} agentes designados`); } catch {}
            return false;
          }
      ...
      if (voiceModeRef.current) {
        ...
        // PATCH_Q: Team/Multi: fala cada mensagem sequencialmente com voz correta por agente
        // Single: só a última; Team/Multi: todos falam na sua vez
        if (destMode === "single" && !hasInlineMention && toSpeak.length > 1) toSpeak = toSpeak.slice(-1);
...
async function stopRealtime(reason = 'client_stop') {
  ...
  function stopTts() {
    ...
    setTtsPlaying(false);
    setSpeakingAgentId(null); // PATCH_UX: clear speaking agent on stop
  }
  ...
  async function playTts(textToSpeak, agentId, opts = {}) {
    ...
    stopTts();
    setTtsPlaying(true);
    setSpeakingAgentId(agentId || null); // PATCH_UX: track speaking agent
    setV2vPhase('playing');
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        audio.onended = () => {
          ...
          setTtsPlaying(false);
          setSpeakingAgentId(null); // PATCH_UX: clear speaking agent
          setV2vPhase(null);
  ...
  return (
    <>
    {/* PATCH_UX_CSS: Keyframes for avatar animations */}
    <style>{`
      @keyframes orkioAvatarPulse {
        0%, 100% { transform: scale(1); opacity: 0.7; }
        50% { transform: scale(1.15); opacity: 1; }
      }
      @keyframes orkioFadeIn {
        from { opacity: 0; transform: translateY(6px); }
        to { opacity: 1; transform: translateY(0); }
      }
      .orkio-msg-fade { animation: orkioFadeIn 0.3s ease-out; }
    `}</style>
    <PWAInstallPrompt />
    {showTermsModal && (
        ...
        <div style={{ ...styles.chatArea, padding: isMobile ? "12px 12px 18px" : styles.chatArea.padding }}>
          {messages.length === 0 ? (
            ...
            messages.map((m) => (
              ...
              >
                {/* PATCH_UX_AVATAR: Premium avatar per-message — larger, speaking glow, fallback initials, status indicator */}
                {m.role === "assistant" && (() => {
                  const msgAgent = m.agent_id ? agents.find(a => String(a.id) === String(m.agent_id)) : null;
                  const avatarUrl = msgAgent?.avatar_url || lastAgentInfo?.avatar_url;
                  const agentName = m.agent_name || msgAgent?.name || "Agent";
                  const isSpeaking = ttsPlaying && speakingAgentId && String(speakingAgentId) === String(m.agent_id);
                  const isStreaming = String(m.id || "").startsWith("tmp-stream-");
                  ...
                  return (
                    <div style={{
                      marginRight: 10, flexShrink: 0, alignSelf: "flex-start", marginTop: 2,
                      position: "relative",
                    }}>
                      {/* Speaking glow ring */}
                      {isSpeaking && (
                        <div style={{
                          ...
                          background: "transparent",
                          border: "2px solid rgba(55,197,255,0.6)",
                          animation: "orkioAvatarPulse 1.2s ease-in-out infinite",
                        }} />
                      )}
                      {avatarUrl ? (
                        <img
                          ...
                          style={{
                            ...
                            border: isSpeaking ? "2.5px solid #37C5FF" : "2.5px solid rgba(255,255,255,0.18)",
                            boxShadow: isSpeaking
                              ? "0 0 16px rgba(55,197,255,0.45), 0 0 32px rgba(55,197,255,0.2)"
                      ...
                      {/* Fallback initials circle (shown when no avatar or on error) */}
                      <div style={{
                        display: avatarUrl ? "none" : "flex",
                        width: 44, height: 44, borderRadius: "50%",
                        background: `linear-gradient(135deg, ${avatarBg}, hsl(${(hue + 40) % 360}, 55%, 52%))`,
                      ...
                      <div style={{
                        ...
                        background: isSpeaking ? "#37C5FF" : isStreaming ? "#F6C453" : "#4ADE80",
                        border: "2px solid #0B0F14",
                        boxShadow: isSpeaking ? "0 0 8px rgba(55,197,255,0.6)" : "none",
